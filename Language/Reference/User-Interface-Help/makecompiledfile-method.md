---
title: MakeCompiledFile method (VBA)
keywords: vbob6.chm102018
f1_keywords:
- vbob6.chm102018
api_name:
- Office.MakeCompiledFile
ms.assetid: c6b2501f-1ae3-c1e7-4df3-ea139ea4d1b0
ms.date: 12/06/2018
ms.localizationpriority: medium
---


# MakeCompiledFile method

Causes the current project to be written as a DLL. The DLL name is specified by the **[BuildFileName](buildfilename-property.md)** property.

## Syntax

_object_.**MakeCompiledFile**

The _object_ placeholder represents an [object expression](../../Glossary/vbe-glossary.md#object-expression) that evaluates to an object in the **Applies To** list.

## See also

- [Collections (Visual Basic Add-In Model)](../visual-basic-add-in-model/collections-visual-basic-add-in-model.md)
- [Visual Basic Add-in Model reference](visual-basic-add-in-model-reference.md)
- [Visual Basic language reference](visual-basic-language-reference.md)

[!include[Support and feedback](~/includes/feedback-boilerplate.md)]