...
In contrast to a  **Page**, a **Tab** does not contain any controls. Controls that appear within the region bounded by a **[TabStrip](tabstrip-control.md)** are contained on the form, as is the **[TabStrip](tabstrip-control.md)**.
Each  **Tab** has its own set of properties, but has no methods or events. You must use events from the appropriate **[TabStrip](tabstrip-control.md)** to initiate processing of an individual **Tab**.
