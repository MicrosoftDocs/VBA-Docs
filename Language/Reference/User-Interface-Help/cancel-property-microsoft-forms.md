...
|**False**|The  **[CommandButton](commandbutton-control.md)** is not the Cancel button (default).|
...
Only one  **[CommandButton](commandbutton-control.md)** on a form can be the Cancel button. Setting **Cancel** to **True** for one command button automatically sets it to **False** for all other objects on the form. When a **CommandButton's Cancel** property is set to **True** and the form is the active form, the user can choose the command button by clicking it, pressing ESC, or pressing ENTER when the button has the [focus](../../Glossary/vbe-glossary.md#focus).
