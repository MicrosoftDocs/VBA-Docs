...
- If the variable being read into is a [Variant](../../Glossary/vbe-glossary.md#Variant) of [numeric type](../../Glossary/vbe-glossary.md#numeric-type), **Get** reads 2 bytes identifying the **VarType** of the **Variant** and then the data that goes into the variable. For example, when reading a **Variant** of **VarType** 3, **Get** reads 6 bytes: 2 bytes identifying the **Variant** as **VarType** 3 (**Long**) and 4 bytes containing the [Long](../../Glossary/vbe-glossary.md#Long) data. The record length specified by the **Len** clause in the **Open** statement must be at least 2 bytes greater than the actual number of bytes required to store the variable.
