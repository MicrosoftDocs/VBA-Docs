---
title: Events (Visual Basic for Applications)
ms.assetid: b5cad0fb-0e27-4cce-8998-eba61b07dad3
ms.date: 12/11/2018
ms.localizationpriority: medium
---


# Events (Visual Basic for Applications)


- [Activate, Deactivate](user-interface-help/activate-deactivate-events.md)    
- [Initialize](user-interface-help/initialize-event.md)   
- [QueryClose](user-interface-help/queryclose-event.md)   
- [Resize](user-interface-help/resize-event.md)   
- [Terminate](user-interface-help/terminate-event-visual-basic-for-applications.md)
    

## See also

- [Events (Microsoft Forms)](user-interface-help/events-microsoft-forms.md)
- [Events (Visual Basic Add-In Model)](visual-basic-add-in-model/events-visual-basic-add-in-model.md)
- [Visual Basic language reference](user-interface-help/visual-basic-language-reference.md)
- [Office client development reference](/office/client-developer/office-client-development)

[!include[Support and feedback](~/includes/feedback-boilerplate.md)]
