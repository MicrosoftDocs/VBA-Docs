...
For example, you could use the **DMin** function in calculated controls on a report to display the smallest order amount for a particular customer. Or you could use the **DMin** function in a query expression to display all orders with a discount greater than the minimum possible discount.
