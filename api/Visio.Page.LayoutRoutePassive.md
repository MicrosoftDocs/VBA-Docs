...
When **LayoutRoutePassive** is set to **True**, advanced connector routing is disabled, which means that when connectors are glued to shapes, they don't attach to the shapes dynamically. That is, when those shapes are moved, the connector endpoints that are glued to the shapes move with them, but without the benefits of dynamic routing, such as shifting their location to a new side of the shape when appropriate, or redistribution (in the Flowchart routing style only).

The default setting of the **LayoutRoutePassive** property is **False**, which means that advanced connector routing is enabled. Because setting the property to **True** disables advanced connector routing, doing so can result in shapes that are glued incorrectly, which, in turn, leads to unsatisfactory diagram appearance and connector behavior. As a result, it is suggested that you set this property to **True** only briefly, when you specifically don't want advanced connector routing to occur.
