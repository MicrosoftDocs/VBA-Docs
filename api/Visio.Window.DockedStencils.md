...
The  **DockedStencils** method returns an array of strings&mdash;the names of the stencils shown in the docked stencil panes of a window. When the window is a drawing window, the number of docked stencil panes ( _n_ ) is equal to or greater than zero, and _n_ is zero when the window isn't a drawing window.
