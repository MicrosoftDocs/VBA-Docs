...
| nEventID| The unique identifier of the **Event** object within the **EventList** collection. (Unlike the **Index** property of the **EventList** collection, nEventID doesn't change as **Event** objects are added to or deleted from the collection.)<br/> Get the **Event** object from **VisEventProc** by using the following code: `pSourceObj .EventList.ItemFromID(nEventID )`|
...
The following example assumes that there is an active document in the Visio application window.
