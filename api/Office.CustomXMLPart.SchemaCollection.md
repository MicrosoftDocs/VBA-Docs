---
title: CustomXMLPart.SchemaCollection property (Office)
keywords: vbaof11.chm295005
f1_keywords:
- vbaof11.chm295005
api_name:
- Office.CustomXMLPart.SchemaCollection
ms.assetid: 6976e6c3-8ae6-b342-5dd7-7eaaa2e902b5
ms.date: 01/07/2019
ms.localizationpriority: medium
---


# CustomXMLPart.SchemaCollection property (Office)

Gets or sets a **[CustomXMLSchemaCollection](office.customxmlschemacollection.md)** object representing the set of schemas attached to a bound region of data in a document. Read/write.


## Syntax

_expression_.**SchemaCollection**

_expression_ An expression that returns a **[CustomXMLPart](Office.CustomXMLPart.md)** object.


## Return value

CustomXMLSchemaCollection


## See also

- [CustomXMLPart object members](overview/library-reference/customxmlpart-members-office.md)

[!include[Support and feedback](~/includes/feedback-boilerplate.md)]