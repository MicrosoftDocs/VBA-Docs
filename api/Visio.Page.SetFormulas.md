...
If the  _sheetID_ in an entry is **visInvalShapeID** (-1) or if the bottom byte of _sectionIdx_ is **visSectionInval** (255), the entry is ignored by the **SetResults** method. This is because the same _SID_SRCStream()_ array can be used on several calls to **SetFormulas**, **GetFormulas**, and similar methods with the caller only needing to make minor changes to the stream between calls.
