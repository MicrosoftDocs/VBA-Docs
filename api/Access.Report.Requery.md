...
If you omit the object specified by _expression_, the **Requery** method requeries the underlying data source for the report that has the focus. If the control that has the focus has a record source or row source, it will be requeried; otherwise, the control's data will simply be refreshed.
