...
| _Domain_|Required|**String**|A string expression that identifies the set of records that constitutes the domain. It can be a table name or a query name for a query that does not require a parameter.|
...
If you simply want to count all records in _domain_ without specifying any restrictions, use the **[Count](../access/concepts/criteria-expressions/count-function-microsoft-access-sql.md)** function. The **Count** function has been optimized to speed counting of records in queries. Use the **Count** function in a query expression instead of the **DCount** function, and set optional criteria to enforce any restrictions on the results. 
...
If _domain_ is a table with a primary key, you can also count the total number of records by setting _expr_ to the primary key field because there will never be a **Null** in the primary key field.
