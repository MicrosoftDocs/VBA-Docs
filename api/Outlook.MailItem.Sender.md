...
In a session where multiple accounts are defined in the profile, you can set this property to specify the account from which to send a mail item. Set this property to the **AdressEntry** object of the user that is represented by the [CurrentUser](Outlook.Account.CurrentUser.md) property of a specific account.
