...
| _Link_|Optional|**MsoTriState**|Determines whether to create a link to the source file of the Clipboard contents. An error occurs if the Clipboard contents don't support a link.|
...
Adds the shape to the collection of shapes in the specified format. If the specified data type is a text data type, a new text box is created with the text. If the paste succeeds, the **PasteSpecial** method returns a **[ShapeRange](PowerPoint.ShapeRange.md)** object representing the shape range that was pasted.
