...
_expression_.**MustFlushScopeBeginning** (_app_)
...
1. **MustFlushScopeBeginning** event - Client should not call methods that have side effects.
    
2. There are zero (0) or more events in the event queue.
    
3. **BeforeShapeDelete** event - Shape is viable, but Visio is going to delete it.
    
4. **MustFlushScopeEnded** event - Client can resume invoking methods that have side effects.
    
5. **ShapesDeleted** event - Shape has been deleted.
    
6. **NoEventsPending** event - No events remain to be fired.
