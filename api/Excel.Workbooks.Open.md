...
| _UpdateLinks_|Optional| **Variant**|Specifies the way external references (links) in the file, such as the reference to a range in the Budget.xls workbook in the following formula `=SUM([Budget.xls]Annual!C10:C25)`, are updated. If this argument is omitted, the user is prompted to specify how links will be updated. For more information about the values used by this parameter, see the Remarks section.<br/><br/>If Microsoft Excel is opening a file in the WKS, WK1, or WK3 format and the UpdateLinks argument is 0, no charts are created; otherwise, Microsoft Excel generates charts from the graphs attached to the file.|
...
| _Editable_|Optional| **Variant**|If the file is a Microsoft Excel 4.0 add-in, this argument is **True** to open the add-in so that it is a visible window. If this argument is **False** or omitted, the add-in is opened as hidden, and it cannot be unhidden. This option does not apply to add-ins created in Microsoft Excel 5.0 or later.<br/><br/>If the file is an Excel template, **True** to open the specified template for editing. **False** to open a new workbook based on the specified template. The default value is **False**.|
