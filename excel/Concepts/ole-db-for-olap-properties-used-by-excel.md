...
|DBPROPSET_DATASOURCEINFO| [MDPROP_NAMED_LEVELS](https://msdn.microsoft.com/library/ms713691.aspx)|Excel checks that the lowest bit is set (MDPROPVAL_NL_NAMEDLEVELS).|If the lowest bit of this property is not set, Excel fails.|
...
| [MDSCHEMA_HIERARCHIES](/previous-versions/sql/sql-server-2012/ms126062(v=sql.110))|HIERARCHY_ORIGIN|MD_ORIGIN_ATTRIBUTE set and not MD_ORIGIN_USER_DEFINED|Excel has special handling of attribute hierarchies in OLAP PivotTables, so attribute hierarchies are marked as such.|
| [MDSCHEMA_HIERARCHIES](/previous-versions/sql/sql-server-2012/ms126062(v=sql.110))|HIERARCHY_DISPLAY_FOLDER||Based on this property, the PivotTable Field List displays hierarchies in folders under their dimensions.|
| [MDSCHEMA_MEASUREGROUPS](/previous-versions/sql/sql-server-2012/ms126178(v=sql.110))|MEASUREGROUP_NAME|| **Measures** are listed in a folder representing their measure group in the PivotTable Field List.|
| [MDSCHEMA_MEASUREGROUPS](/previous-versions/sql/sql-server-2012/ms126178(v=sql.110))|MEASUREGROUP_CAPTION|| **Measures** are listed in a folder representing their measure group with this caption in the PivotTable Field List.|
| [MDSCHEMA_SETS](/previous-versions/sql/sql-server-2012/ms126290(v=sql.110))|SET_DISPLAY_FOLDER||Excel reads the display folder property to enable it to place sets in display folders in the PivotTable Field List.|
| [MDSCHEMA_SETS](/previous-versions/sql/sql-server-2012/ms126290(v=sql.110))|SET_CAPTION||Excel reads the set caption for displaying in the PivotTable report and in the PivotTable Field List.|
| [MDSCHEMA_KPIS](/previous-versions/sql/sql-server-2012/ms126258%28v%3dsql.110%29)|KPI_DISPLAY_FOLDER||KPIs (key performance indicators) defined on the server are listed in the PivotTable field list, and the components (value, goal, status, and trend) can be added to the values area. Excel reads this property to place the KPI in the correct display folder in the PivotTable Field List.|
| [MDSCHEMA_KPIS](/previous-versions/sql/sql-server-2012/ms126258%28v%3dsql.110%29)|KPI_PARENT_KPI_NAME||Excel reads this property to place child KPIs in subfolders under their parent KPI in the PivotTable Field List (if display folders are defined, those are used instead).|
| [MDSCHEMA_KPIS](/previous-versions/sql/sql-server-2012/ms126258%28v%3dsql.110%29)|KPI_TREND_GRAPHIC||Excel reads this property and, based on the value, maps it to the closest conditional formatting icon set in Excel when Trend is added to the PivotTable.|
| [MDSCHEMA_KPIS](/previous-versions/sql/sql-server-2012/ms126258%28v%3dsql.110%29)|KPI_STATUS_GRAPHIC||Excel reads this property and, based on the value, maps it to the closest conditional formatting icon set in Excel when Status is added to the PivotTable.|
| [MDSCHEMA_ACTIONS](/previous-versions/sql/sql-server-2012/ms126032(v%3dsql.110))|||Additional Actions feature. Excel exposes server-defined actions in the shortcut menu of an OLAP PivotTable report when actions exist on the server for the selected context.|
| [MDSCHEMA_MEASURES](/previous-versions/sql/sql-server-2012/ms126250(v%3dsql.110))|MEASURE_DISPLAY_FOLDER||Read by Excel so it can place measures in the correct display folder in the PivotTable Field List.|
| [MDSCHEMA_MEASURES](/previous-versions/sql/sql-server-2012/ms126250(v%3dsql.110)))|EXPRESSION||Read by Excel to determine whether a measure is calculated. If it is a string and not empty, Excel considers it a calculated measure.|
| [MDSCHEMA_PROPERTIES](/previous-versions/sql/sql-server-2012/ms126309(v=sql.110))|PROPERTY_NAME|"MEMBER_VALUE" This schema also used for getting regular member properties. The "MEMBER_VALUE" value is a special case, but there are other usage.|Excel gets the member value property of the key attribute in a dimension by restricting to "MEMBER_VALUE" in the PROPERTY_NAME column.If the data type (DATA_TYPE) of the MEMBER_VALUE property of the key attribute of a Time dimension is **Date**, the PivotTable exposes date filtering instead of label filtering. The actual date filtering is done based on the member value property of the key independent of which hierarchy of that dimension is filtered.<table><tr><th>**Note**</th></tr><tr><td>Date filtering requires support for subselects (see MDPROP_MDX_SUBQUERIES above).</td></tr></table>|
...
| [MDSCHEMA_LEVELS](/previous-versions/sql/sql-server-2012/ms126038(v=sql.110))|LEVEL_ATTRIBUTE_HIERARCHY_NAME||Used by Excel to hide special grouping levels with system-generated names. Note that this is not needed with Microsoft SQL Server 2005 Analysis Services Service Pack 2.|
| [MDSCHEMA_LEVELS](/previous-versions/sql/sql-server-2012/ms126038(v=sql.110))|CUSTOM_ROLLUP_SETTINGS|0|If not 0, Excel assumes the level has custom rollup. Excel checks this for all levels of each hierarchy, and if custom rollup is present, some operations are disabled (such as grouping).|
...
Excel uses this property to determine which currency symbol to use when formatting values with `FORMAT_STRING` set to **Currency**. For more information, see [Retrieving cell properties](/previous-versions/windows/desktop/ms715853(v=vs.85)).
