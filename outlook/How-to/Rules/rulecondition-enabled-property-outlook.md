...
Returns or sets a  **Boolean** that determines if the **[RuleCondition](../../../api/Outlook.RuleCondition.md)** is enabled. Read/write.
...
After you enable a rule condition, you must also save the rule by using  **[Rules.Save](../../../api/Outlook.Rules.Save.md)** so that the rule condition and its enabled state will persist beyond the current session. A rule condition is only enabled after it has been saved successfully.

Returns an error if you attempt to enable a rule condition that is supported only on a rule of type  **olRuleSend** for a rule of type **olRuleReceive** , or vice versa. For more information on suppport by rules for receiving messages or rules for sending messages, see[Specify Rule Conditions](./specifying-rule-conditions.md).
...
[RuleCondition Object](../../../api/Outlook.RuleCondition.md)
